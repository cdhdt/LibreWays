...
- **What it can learn:** which external services the device talks to and when; without a relay,
  ...
  third-party providers' own knowledge of the query. With a relay, it is **designed** to learn only
  that the device is talking to the relay (e.g. a Tor entry node or proxy), not to which provider or
  with what content — **but this destination-hiding property is not yet verified for this app's
  stack, and this adversary is exactly the one who would see the leak if it exists.** Decision D18
  (`docs/adr/007-relay-and-proxy.md`) records, as a blocking requirement, that Java/Android SOCKS
  proxying can resolve the destination hostname locally by default: if that happens, this adversary
...
- **Residual risk:** traffic-timing/size correlation against a relay (including Tor) by a
  ...
  when the relay does correctly hide the destination — and, separately and more urgently, may learn
  the destination itself outright if the DNS-leak risk above turns out not to be preventable with
  this stack, in which case decision D18 is reopened rather than this document being left to overstate
  what the relay achieves.
